...
const MD_PATH = process.env.STUDENT_CAREER_AI_TRACKER || 'data/applications.md';
const DB_PATH = process.env.STUDENT_CAREER_AI_TRACKER_DB
  || (MD_PATH.endsWith('.md') ? MD_PATH.slice(0, -3) + '.db' : MD_PATH + '.db');

// SQLite must never open the source of truth itself (an explicit
// STUDENT_CAREER_AI_TRACKER_DB could point both names at the same file).
...
function loadStates() {
  if (!existsSync(STATES_PATH)) {
    console.error(`Error: ${STATES_PATH} not found — cannot validate statuses. Run from the student-career-ai root.`);
    process.exit(1);
  }
