 ...
 * next `/student-career-ai pipeline` run both re-surface them, and they get evaluated
...
function resolveInsideRepo(inputPath, fallbackPath, flag) {
  ...
  try {
    repoReal = realpathSync(STUDENT_CAREER_AI);
...
const defaultPipeline = existsSync(join(STUDENT_CAREER_AI, 'data/pipeline.md'))
  ? join(STUDENT_CAREER_AI, 'data/pipeline.md')
  : join(STUDENT_CAREER_AI, 'pipeline.md');
const PIPELINE_FILE = resolveInsideRepo(argValue('--pipeline'), defaultPipeline, '--pipeline');
const STATE_FILE = resolveInsideRepo(argValue('--state'), join(STUDENT_CAREER_AI, 'batch/batch-state.tsv'), '--state');
const REPORTS_DIR = join(STUDENT_CAREER_AI, 'reports');
...
for (let i = pendStart + 1; i < pendEnd; i++) {
  ...
  const reportLink = normalizeReportLink(`[${num}](reports/${reportFile})`, dirname(PIPELINE_FILE), STUDENT_CAREER_AI);
