 ...
 * becomes 1.
 *
 * Why patch `dns.lookup` rather than configure the HTTP client: student-career-ai
 ...
 *   - `STUDENT_CAREER_AI_NO_DNS_CACHE=1` opts out entirely — no memoization AND no
 *     pacing, since both live inside this patched lookup.
 *   - `STUDENT_CAREER_AI_DNS_LOOKUPS_PER_MIN` caps resolver-bound lookups
...
export function lookupsPerMinFromEnv(env = process.env) {
  const raw = env.STUDENT_CAREER_AI_DNS_LOOKUPS_PER_MIN;
  ...
  if (!Number.isFinite(n) || n < 0) {
    console.error(
      `⚠ STUDENT_CAREER_AI_DNS_LOOKUPS_PER_MIN=${raw} is not a non-negative number — `
...
if (process.env.STUDENT_CAREER_AI_NO_DNS_CACHE !== '1') {
