...
/** Find a registry entry by bare id, full name, or `student-career-ai-plugin-<id>`. */
export function findInRegistry(root, nameOrId) {
  const reg = loadRegistry(root);
  return reg.plugins.find(p =>
    p.id === nameOrId || p.name === nameOrId || p.name === `student-career-ai-plugin-${nameOrId}`) || null;
}
...
export function validateRegistryEntry(e, { idRe, hookKinds, reservedEnv }) {
  const errs = [];
  if (typeof e.name !== 'string' || !e.name.startsWith('student-career-ai-plugin-')) errs.push('name must start with "student-career-ai-plugin-"');
  if (typeof e.id !== 'string' || !idRe.test(e.id)) errs.push('invalid id');
  if (e.name && e.id && e.name !== `student-career-ai-plugin-${e.id}`) errs.push('name must equal student-career-ai-plugin-<id>');
