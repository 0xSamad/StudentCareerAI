...
eq('duplicate pair: one quality.duplicates entry', dupPair.quality.duplicates,
  [{ uid: 'studentcareer-jane-doe-bc225ac5--acme-293abb6b', name: 'Jane Doe', company: 'Acme', count: 2 }]);
...
ok('UID = studentcareer-{uidPart(name)}--{uidPart(company)}', /UID:studentcareer-jane-doe-[0-9a-f]{8}--acme-[0-9a-f]{8}\r\n/.test(contactToVcard(jane, { rev: REV })));
...
eq('CJK contact UID: same input -> same UID across two calls',
  ...
  contactToVcard(taro, { rev: REV }).match(/UID:[^\r]+/)[0]);
ok('CJK contact UID = studentcareer-{8-hex}--globex-{8-hex}', /UID:studentcareer-[0-9a-f]{8}--globex-[0-9a-f]{8}/.test(contactToVcard(taro, { rev: REV })));
...
ok('both-non-ASCII name+company UID is dual-bare studentcareer-{8-hex}--{8-hex}',
  /^studentcareer-[0-9a-f]{8}--[0-9a-f]{8}$/.test(contactToVcard(taroKk, { rev: REV }).match(/UID:([^\r]+)/)[1]));
ok('ASCII UID = studentcareer-jane-doe-{8-hex}--acme-{8-hex}', /^studentcareer-jane-doe-[0-9a-f]{8}--acme-[0-9a-f]{8}$/.test(contactUid(jane)));
...
const expectedCard = [
  'BEGIN:VCARD',
  'VERSION:3.0',
  'UID:studentcareer-jane-doe-bc225ac5--acme-293abb6b',
  'FN:Jane Doe',
  'N:Doe;Jane;;;',
  'ORG:Acme',
...
// copy of the script into a temp dir is a fully isolated student-career-ai root:
...
try {
  ...
  ok('written vcf carries UIDs', /UID:studentcareer-jane-doe-[0-9a-f]{8}--acme-[0-9a-f]{8}/.test(written));
