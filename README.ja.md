...
<p align="center">
  <a href="https://x.com/santifer"><img src="docs/hero-banner.jpg" alt="student-career-ai マルチエージェント求職システム" width="800"></a>
</p>
...
<p align="center">
  <img src="docs/demo.gif" alt="student-career-ai デモ" width="800">
</p>
...
student-career-aiは、あらゆるAIコーディングCLIを本格的な求職コマンドセンターに変えます。スプレッドシートで応募を手動管理する代わりに、AIによる以下のパイプラインが手に入ります:
...
> **重要: これは「とにかく数を撃つ」ツールではありません。** student-career-aiはフィルターです -- 何百もの求人の中から、あなたの時間を割く価値のある数件を見つけ出すためのツールです。本システムは4.0/5未満のスコアの求人への応募を強く非推奨としています。あなたの時間もリクルーターの時間も貴重です。送信前に必ず内容を確認してください。

student-career-aiはエージェンティックです: Claude CodeがPlaywrightで求人ページを操作し、（キーワードマッチではなく）あなたのCVと求人票を突き合わせて適合度を推論し、求人ごとにレジュメを最適化します。
...
このシステムは、740件以上の求人を評価し、100件以上のテーラーメイドCVを生成し、Head of Applied AIのポジションを獲得した人物によって作られました。[詳細なケーススタディを読む](https://santifer.io/student-career-ai-system)。
...
これにより最新リリースが `./student-career-ai` にクローンされ、依存関係がインストールされます。その後:
...
**初回起動時、student-career-aiが対話するだけでセットアップ（CV、プロフィール、対象ロール）をご案内します。手で編集するものは何もありません。**
...
student-career-aiは複数のモードを持つ単一のスラッシュコマンドです:

```
/student-career-ai                → 利用可能なすべてのコマンドを表示
/student-career-ai {求人票を貼る}  → 完全自動パイプライン（評価 + PDF + トラッカー）
/student-career-ai scan           → ポータルをスキャンして新しい求人を探す
/student-career-ai pdf            → ATS最適化CVを生成
/student-career-ai batch          → 複数オファーをバッチ評価
/student-career-ai tracker        → 応募ステータスを表示
/student-career-ai apply          → AIで応募フォームを入力
/student-career-ai pipeline       → 保留中のURLを処理
/student-career-ai contacto       → LinkedInアウトリーチメッセージ
/student-career-ai deep           → 企業の深掘りリサーチ
/student-career-ai training       → コース/資格を評価
/student-career-ai project        → ポートフォリオプロジェクトを評価
```

または、単に求人URLや記述を直接貼り付けるだけ -- student-career-aiが自動検知してフルパイプラインを実行します。
...
Santiagoです -- Head of Applied AI、元創業者（自分の名前を冠した事業を立ち上げて売却、その事業は今も稼働中）。student-career-aiは自分自身の求職活動を管理するために作りました。結果、現職を獲得することに成功しました。
...
**student-career-aiはローカルで動作するオープンソースツールです — ホステッドサービスではありません。** 本ソフトウェアを使用することにより、以下を承諾したものとみなされます:
