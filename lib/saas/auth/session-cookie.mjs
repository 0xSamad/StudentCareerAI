 ...
 * Honor COOKIE_SECURE, then the request protocol, else NODE_ENV.
 */
export function resolveCookieSecure(request) {
...
export function clearSessionCookie(options = {}) {
  const { request, secure = resolveCookieSecure(request), sameSite = "Lax", path = "/" } = options;
  const parts = [
    `${SESSION_COOKIE_NAME}=`,
    `Path=${path}`,
    "HttpOnly",
