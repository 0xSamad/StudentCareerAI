 ...
 *
 * Provisions a throwaway candidates file via STUDENT_CAREER_AI_REPLY_CANDIDATES and a
...
function runFile(candidatesPath, filePath, extraEnv = {}) {
  return execFileSync(NODE, [CLI, '--file', filePath], {
    cwd: ROOT,
    env: { ...process.env, STUDENT_CAREER_AI_REPLY_CANDIDATES: candidatesPath, ...extraEnv },
    encoding: 'utf8',
...
console.log('8. interactive (stdin) mode — no --file flag');
{
  ...
  const out = execFileSync(NODE, [CLI], {
    cwd: ROOT,
    env: { ...process.env, STUDENT_CAREER_AI_REPLY_CANDIDATES: candidates },
    input: stdin,
    encoding: 'utf8',
    stdio: ['pipe', 'pipe', 'pipe'],
  });
