 ...
 *
 * Tests provision a throwaway tracker via the STUDENT_CAREER_AI_TRACKER /
 * STUDENT_CAREER_AI_TRACKER_LOCK env overrides (same sandbox pattern as
...
function makeSandbox(trackerContent) {
  ...
  // The lock env value must live under tmpdir and use the student-career-ai prefix
...
// ── 14. Lock timeout: structured exit 4, JSON error, no write ──
{
  ...
  const r = runSetStatus(['2', 'Applied', '--json'], sb, { STUDENT_CAREER_AI_TRACKER_LOCK_TIMEOUT_MS: '300' });
  let parsed = null;
  try { parsed = JSON.parse(r.stdout); } catch {}
...
// ── 14b. Non-timeout lock failure is NOT reported as lock-timeout ─
{
  ...
  // timeouts.
  const blocker = join(sb.dir, 'student-career-ai-merge-tracker-blocker');
...
// ── 15. Orphaned recovery guard does not block stale-lock recovery ─
{
  ...
  const lockDir = join(dir, 'student-career-ai-merge-tracker-guardtest.lock');
