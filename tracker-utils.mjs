 ...
 * `applications.md` (original root layout). The `STUDENT_CAREER_AI_TRACKER` env var
 ...
 * on one spelling.
 *
 * @param {string} rootDir - The student-career-ai repository root.
 * @returns {string} Absolute canonical tracker path.
 */
export function resolveTrackerPath(rootDir) {
  const raw = process.env.STUDENT_CAREER_AI_TRACKER
 ...
 * that pointing `STUDENT_CAREER_AI_TRACKER` at another workspace moves the whole set
 ...
 * a tracker. `STUDENT_CAREER_AI_PDF_INDEX` overrides it explicitly.
 *
 ...
 * that resolve the tracker from `STUDENT_CAREER_AI_TRACKER` then read one workspace's
...
export function resolvePdfIndexPath(trackerPath) {
  return process.env.STUDENT_CAREER_AI_PDF_INDEX
 ...
 * contends on the same lock. `STUDENT_CAREER_AI_TRACKER_LOCK` exists for tests and
 ...
 * use the student-career-ai lock-name prefix. Invalid values are ignored and the
...
export function trackerLockDirFor(appsFile) {
  ...
  const fallback = join(tmpRoot, `student-career-ai-merge-tracker-${lockKey}.lock`);
  const envValue = process.env.STUDENT_CAREER_AI_TRACKER_LOCK;
  ...
  if (!basename(candidate).startsWith('student-career-ai-merge-tracker-')) return fallback;
  return candidate;
}
