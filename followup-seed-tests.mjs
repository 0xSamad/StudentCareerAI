...
// its cadence at module load from STUDENT_CAREER_AI_PROFILE and otherwise falls back
...
// do nothing.
process.env.STUDENT_CAREER_AI_PROFILE = DEFAULT_CADENCE_PROFILE;
...
function makeSandbox() {
  ...
  const lock = join(dir, `student-career-ai-followups-test-${Math.random().toString(36).slice(2)}.lock`);
...
function run(args, sandbox, extraEnv = {}) {
  const env = {
    ...
    STUDENT_CAREER_AI_FOLLOWUPS: sandbox.followups,
    // Explicit, not inherited: the child resolves its cadence from this or
    // falls back to the user's real config/profile.yml (#2268).
    STUDENT_CAREER_AI_PROFILE: DEFAULT_CADENCE_PROFILE,
    STUDENT_CAREER_AI_FOLLOWUPS_LOCK: sandbox.lock,
    ...extraEnv,
  };
  try {
...
// ── Test 9: lock held by a live pid → exit 4 ────────────────────────────────
{
  ...
  const res = run(['1'], sb, { STUDENT_CAREER_AI_FOLLOWUPS_LOCK_TIMEOUT_MS: '200' });
...
// guard out for exactly this reason; followup-seed.mjs was the outlier.
{
  ...
  const res = run(['1'], sb, {
    STUDENT_CAREER_AI_FOLLOWUPS_LOCK_STALE_MS: '10',
...
// enough to reclaim.
{
  ...
  const res = run(['1'], sb, {
    STUDENT_CAREER_AI_FOLLOWUPS_LOCK_STALE_MS: '60000',
    STUDENT_CAREER_AI_FOLLOWUPS_LOCK_TIMEOUT_MS: '400',
  });
...
// straight into the read-check-append critical section beside its owner.
{
  ...
  const res = run(['1'], sb, {
    STUDENT_CAREER_AI_FOLLOWUPS_LOCK_STALE_MS: '10',
    STUDENT_CAREER_AI_FOLLOWUPS_LOCK_TIMEOUT_MS: '300',
  });
...
// an ownerless lock age out would deadlock every writer behind a real orphan.
{
  ...
  const res = run(['1'], sb, {
    STUDENT_CAREER_AI_FOLLOWUPS_LOCK_STALE_MS: '10',
    STUDENT_CAREER_AI_FOLLOWUPS_LOCK_TIMEOUT_MS: '2000',
  });
