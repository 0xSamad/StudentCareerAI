 ...
 *
 * Provisions a throwaway queue via STUDENT_CAREER_AI_INBOX and a temp CWD; never
...
function run(inbox, args, opts = {}) {
  return execFileSync(NODE, [CLI, ...args], {
    cwd: ROOT,
    env: { ...process.env, STUDENT_CAREER_AI_INBOX: inbox },
    encoding: 'utf8',
...
console.log('6. first add on the default path self-heals .gitignore (idempotent)');
{
  ...
  const addOnce = () => execFileSync(NODE, [CLI, 'add', 'queue a scan'], {
    cwd: repo, env: { ...process.env, STUDENT_CAREER_AI_INBOX: '' }, stdio: ['pipe', 'pipe', 'pipe'],
  });
  addOnce(); addOnce();
