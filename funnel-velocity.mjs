...
const STATES_FILE = join(STUDENT_CAREER_AI, 'templates/states.yml');
...
export function loadBenchmarks(explicitPath) {
  const path = explicitPath
    || (existsSync(join(STUDENT_CAREER_AI, 'config/benchmarks.yml')) ? join(STUDENT_CAREER_AI, 'config/benchmarks.yml') : join(STUDENT_CAREER_AI, 'templates/benchmarks.yml'));
  let doc;
  try {
...
function selfTest() {
  ...
  const bm = loadBenchmarks(join(STUDENT_CAREER_AI, 'templates/benchmarks.yml')).benchmarks;
...
function main() {
  ...
  const states = loadCanonicalStates(STATES_FILE);
  const trackerPath = resolveTrackerPath(STUDENT_CAREER_AI);
