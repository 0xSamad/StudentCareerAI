...
const SET_STATUS_SCRIPT = join(STUDENT_CAREER_AI, 'set-status.mjs');
const ARCHIVE_POSTING_SCRIPT = join(STUDENT_CAREER_AI, 'archive-posting.mjs');
...
const appsFile = resolveTrackerPath(STUDENT_CAREER_AI);
...
try {
  const statusOutput = execFileSync(NODE, setStatusArgs, { cwd: STUDENT_CAREER_AI, env: process.env, encoding: 'utf-8' });
