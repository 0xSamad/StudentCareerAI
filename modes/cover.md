...
Works in two modes:
- **Slug mode:** `/student-career-ai cover {slug}` — loads the existing evaluation report draft as a starting point
- **Paste mode:** `/student-career-ai cover` or JD pasted directly — starts from scratch
...
When invoked as `/student-career-ai cover {slug}`:
