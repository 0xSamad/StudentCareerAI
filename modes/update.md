...
When the user runs `/student-career-ai update`, execute this interactive update flow.

## Step 1 — Check for Updates

Run `node update-system.mjs check` and parse the JSON output.

- If `up-to-date`: Tell the user "student-career-ai is up to date (v{version})." and stop.
...
```bash
git fetch https://github.com/0xSamad/StudentCareerAI.git main || {
...
5. **Check for new mode files**: If new modes were added (files in `modes/` that don't exist locally), mention them:
   > "✨ New modes available: {list}. Run `/student-career-ai` to see all commands."

## Step 4 — Confirm and Apply

Ask the user for confirmation:
> "Ready to update. Apply changes? (This can be rolled back with `/student-career-ai update rollback`)"
...
5. Now check the exit code captured in step 3:
   - If non-zero, treat apply as failed. Show the captured output and offer:
     > "⚠️ Update apply failed. Want me to show the full error, or try `/student-career-ai update rollback`?"
...
6. Run `node doctor.mjs` to validate the installation
   - If the command exits with a non-zero code, treat validation as failed. Show the captured output and offer:
     > "⚠️ Validation failed after update. Want me to show the full error, or roll back with `/student-career-ai update rollback`?"
...
8. Show final status:
   ...
   If the updater's output ended with its note about the StudentCareerAI Manifesto, relay it once (do not drop it when summarizing):
   > "One more thing: this project ships with the StudentCareerAI Manifesto — a new way of job searching is taking shape, and you are already practicing it. Run `npm run manifesto` to read it and sign it if you want to help. No action needed."

If no:
1. Run `node update-system.mjs dismiss`
2. Tell the user they can run `/student-career-ai update` anytime to check again.

## Step 5 — Rollback (if requested)

If the user says "rollback" or runs `/student-career-ai update rollback`:
