...
Pattern credit: [MadsLorentzen/ai-job-search](https://github.com/MadsLorentzen/ai-job-search)'s `/upskill`, adapted to student-career-ai' tracker and A–F scoring model.
...
- **Scope boundary:** the plan LINKS each resource to `/student-career-ai training {name}` for a full judging pass; it never runs training's 6-dimension scoring itself. `upskill` finds; `training` judges.
...
→ To judge one of these against your profile, run `/student-career-ai training {resource name}`.
...
8. **Scope boundary.** Plan entries link to `/student-career-ai training {name}` for judging a specific resource; the plan itself never runs training's 6-dimension scoring. `upskill` finds; `training` judges.
