...
If deeper company research is useful, recommend running `/student-career-ai deep` separately after the evaluation.
...
After saving the report and recording in the tracker, append a cover letter draft to the report file under `## Cover Letter Draft`. This is a starting point — not the final letter. The user completes it via `/student-career-ai cover {slug}`.
...
> Draft generated at evaluation time. Complete via `/student-career-ai cover {slug}` to fill in angles, confirm research, and generate the PDF.
...
---
*Run `/student-career-ai cover {slug}` to complete angles, confirm company research, and generate the PDF.*
```
