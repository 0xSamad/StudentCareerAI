...
the public JSON APIs student-career-ai already supports — Greenhouse, Ashby, Lever, and
...
- `portals.yml` — dedupe target and write destination (user layer). Honors the
  `STUDENT_CAREER_AI_PORTALS` env override for scratch/testing.
...
- **Genuinely unknown**: for a JS-only portal with no ATS API, paste a specific
  JD into `data/pipeline.md` and run `/student-career-ai pipeline`.

## Step 3 — Handoff

After writing, tell the user to run `/student-career-ai scan` (or a regional preset
