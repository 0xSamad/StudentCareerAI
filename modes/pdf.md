...
20. **Hiring-manager audit — off by default, opt-in only.** Run `modes/pdf/hm-audit.md` if and only if one of these is true; otherwise skip straight to Step 21 without prompting.
    - The invocation carried `--hm-audit` (`/student-career-ai pdf --hm-audit`, or the same flag on a natural-language request).
...
Local photo paths may be absolute or relative to the student-career-ai project root.
...
- Or run `/student-career-ai cover {slug}` later
```
