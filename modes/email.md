...
1. `/student-career-ai email {report-number-or-slug}`
   ...
   - If `data/pdf-index.tsv` contains a PDF for that report, mention it as the CV
     ...
     generated first via `/student-career-ai pdf {slug}` or attached manually.

2. `/student-career-ai email {pasted JD}`
...
4. `/student-career-ai email stuck {report-number-or-slug}`
...
5. `/student-career-ai email noshow {report-number-or-slug}`
   - Load the matching `reports/{NNN}-*.md` for company and role context.
...
- `< 4.0`: restrained; do not oversell. If below 4.0, warn the user before
  drafting that student-career-ai normally recommends against applying.
