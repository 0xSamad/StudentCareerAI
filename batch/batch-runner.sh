...
# student-career-ai batch runner — standalone orchestrator for claude -p workers
...
usage() {
  cat <<'USAGE'
student-career-ai batch runner — process job offers in batch via claude -p workers
...
main() {
  ...
  echo "=== student-career-ai batch runner ==="
  if (( LIMIT > 0 )); then
