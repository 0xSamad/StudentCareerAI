#!/usr/bin/env node
/**
 * gemini-eval.mjs — Gemini-powered Job Offer Evaluator for student-career-ai
 *
...
const PATHS = {
  ...
  evaluate:    join(ROOT, '.claude', 'skills', 'student-career-ai', 'SKILL.md'),
  cv:          join(ROOT, 'cv.md'),
...
if (args.length === 0 || args[0] === '--help' || args[0] === '-h') {
  console.log(`
╔══════════════════════════════════════════════════════════════════╗
║           student-career-ai — Gemini Evaluator (free-tier)             ║
╚══════════════════════════════════════════════════════════════════╝
...
function validateEvaluationShape(text) {
  ...
  if (issues.length > 0) {
    throw new Error(`Gemini returned an invalid student-career-ai report: ${issues.join('; ')}`);
  }
}
...
// ---------------------------------------------------------------------------
console.log('\n' + '═'.repeat(66));
console.log('  STUDENT-CAREER-AI EVALUATION — powered by Google Gemini');
