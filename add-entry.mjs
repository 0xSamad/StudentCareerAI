#!/usr/bin/env node
/**
 * add-entry.mjs — Deterministic dedup + insertion for `/student-career-ai add`.
 *
 ...
 *
 * Test isolation: STUDENT_CAREER_AI_CV / STUDENT_CAREER_AI_ARTICLE_DIGEST override the target
...
const CV_FILE = process.env.STUDENT_CAREER_AI_CV || join(STUDENT_CAREER_AI, 'cv.md');
const ARTICLE_DIGEST_FILE = process.env.STUDENT_CAREER_AI_ARTICLE_DIGEST || join(STUDENT_CAREER_AI, 'article-digest.md');
...
export function applyAdd(payload, { cvText = null, articleText = null } = {}) {
  ...
  if (payload.articleDigest) {
    ...
    const current = articleText === null
      ? '# Article Digest -- Proof Points\n\nCompact proof points from portfolio projects. Read by student-career-ai at evaluation time.\n'
      : articleText;
