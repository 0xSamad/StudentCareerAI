 ...
 *
 * Usage:
 *   node student-career-ai/generate-pdf.mjs <input.html> <output.pdf> [--format=letter|a4] [--report=NNN] [--allow-reorder] [--max-pages=N] [--strict-pages]
 *
 ...
 * or outside the student-career-ai repository.
 *
 ...
 * student-career-ai root with forward slashes. One row per PDF path; when a report
...
async function generatePDF() {
  ...
  if (!inputPath || !outputPath) {
    ...
    console.error('mechanical markdown-to-HTML step by design. Run `/student-career-ai pdf` in your AI');
...
export async function renderHtmlToPdf(html, outputPath, opts = {}) {
  ...
  const tmpHtmlPath = resolve(baseDir, `.student-career-ai-render-${randomUUID()}.html`);
