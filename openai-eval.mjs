#!/usr/bin/env node
/**
 * openai-eval.mjs — OpenAI-compatible Job Offer Evaluator for student-career-ai
 *
...
if (args.length === 0 || args[0] === '--help' || args[0] === '-h') {
  console.log(`
╔══════════════════════════════════════════════════════════════════╗
║       student-career-ai — OpenAI-compatible Evaluator (any endpoint)     ║
╚══════════════════════════════════════════════════════════════════╝
  ...
  OPTIONS
    ...
    --model <id>     Model id            (env OPENAI_MODEL, default gpt-5.6-luna)
...
// Parse flags
let jdText     = '';
let modelName  = process.env.OPENAI_MODEL || 'gpt-5.6-luna';
...
const { contextBody, budgetReport } = buildBudgetedPrompt({
  ...
  jdText,
  noCompress,
  maxTokens: 128_000, // conservative prompt budget (gpt-5.6-luna supports a larger window)
});
...
const systemPrompt = `You are student-career-ai, an AI-powered job search assistant.
...
// ---------------------------------------------------------------------------
console.log('\n' + '═'.repeat(66));
console.log('  STUDENT-CAREER-AI EVALUATION — powered by ' + modelName + ' (' + endpointHost + ')');
