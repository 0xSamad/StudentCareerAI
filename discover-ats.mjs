#!/usr/bin/env node
/**
 * discover-ats.mjs — Company-list → scannable ATS board resolver for student-career-ai
 *
 * Takes a list of companies and resolves each to a scannable ATS board by
 * probing the public JSON APIs student-career-ai already supports (Greenhouse, Ashby,
 ...
 *
 * Probing hits live third-party APIs, so honor STUDENT_CAREER_AI_PORTALS to point at a
 * scratch portals file during tests/experiments.
 *
 * Issue #1864 — github.com/0xSamad/StudentCareerAI
 */
...
const PORTALS_PATH = process.env.STUDENT_CAREER_AI_PORTALS || join(STUDENT_CAREER_AI, 'portals.yml');
...
function printSummary({ resolved, unresolved, duplicates }) {
  console.log(`\n${'='.repeat(78)}`);
  console.log('  ATS Discovery — student-career-ai');
...
async function main() {
  ...
  // rest of student-career-ai treats user files (see DATA_CONTRACT.md).
  let written = false;
