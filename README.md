...
# StudentCareer AI — Autonomous Student Job & Internship Platform

> **Autonomous student career acceleration pipeline.** Designed specifically for university students and graduating seniors to discover, verify eligibility, tailor CVs with zero fabrication, and safely apply to internships and software engineering roles.

[![Capstone Demo](https://img.shields.io/badge/Capstone-Verified_Demo-success)](docs/CAPSTONE_DEMO.md)
[![Project Differentiation](https://img.shields.io/badge/Architecture-Differentiation-blue)](docs/PROJECT_DIFFERENTIATION.md)
[![Test Suites](https://img.shields.io/badge/Tests-76%2B_Passing-emerald)](tests/)
[![Default Mode](https://img.shields.io/badge/Mode-Internships_(Default)-orange)](web/)

---

## 🚀 Quick Capstone Demonstration

Run the reproducible, zero-cost 21-stage student workflow demonstration in Safe DRY-RUN mode:

```bash
node demo-capstone.mjs
```

Launch the official web dashboard:

```bash
cd web
npm run dev
```
Open **[http://localhost:3000](http://localhost:3000)** (Default: **Internships Portal**).

---

## 🎓 The 21-Stage Student Pipeline

```
1. Create student profile (GPA, Graduation Date, Coursework, STAR projects)
2. Upload master CV (Ground truth reference)
3. Select search mode (Default: INTERNSHIPS for students)
4. Configure preferences & safety switches
5. Start agent (RUNNING / PAUSED / STOPPED state machine)
6. Discover opportunities across ATS feeds (Greenhouse, Ashby, Lever)
7. Deduplicate opportunities against history
8. Classify opportunities (INTERNSHIP vs JOB)
9. Check eligibility FIRST (Hard gates: graduation window, degree, work authorization)
10. Reject ineligible opportunities immediately (stops processing)
11. Score eligible opportunities (6-dimension student weighted model)
12. Rank opportunities by compatibility
13. Tailor CV (Strict zero-fabrication contract against ground-truth facts)
14. Generate tailored cover letter
15. Generate confident form answers (high-confidence threshold + sensitive category check)
16. Open application session
17. Fill form fields automatically
18. Validate all required inputs & attachments
19. Submit according to settings (Safe DRY-RUN: AUTO_SUBMIT=false by default)
20. Track result in queue & applications tracker
21. Continue discovering loop & update real-time dashboard
```
...
- 📖 **[Capstone Demonstration Guide](docs/CAPSTONE_DEMO.md)** — Step-by-step walkthrough of *"Find suitable AI/ML internships for this student"*.
- 🏗️ **[Project Architecture & Differentiation](docs/PROJECT_DIFFERENTIATION.md)** — Detailed comparison of inherited, modified, and newly developed modules.
