...
async function main() {
  ...
  for (const name of opts.ats) {
    ...
    if (resolverOutage) {
      ...
      log(`     STUDENT_CAREER_AI_NO_DNS_CACHE=1 only if you know the cache is at fault.`);
  ...
  if (pacing.delayed > 0) {
    log(`DNS pacing:         ${pacing.delayed} lookup${pacing.delayed === 1 ? '' : 's'} delayed, ${Math.round(pacing.waitedMs / 1000)}s total wait (STUDENT_CAREER_AI_DNS_LOOKUPS_PER_MIN to tune, 0 disables)`);
  }
