...
// launchWriter() is the child's STUDENT_CAREER_AI_TRACKER_LOCK_TIMEOUT_MS, and
...
async function runWhileLocked({
  ...
  const dir = mkdtempSync(join(tmpdir(), 'student-career-ai-writer-lock-'));
  const tracker = join(dir, 'applications.md');
  const lockDir = join(dir, `student-career-ai-merge-tracker-${name}.lock`);
  ...
  const childEnv = {
    ...process.env,
    STUDENT_CAREER_AI_TRACKER: tracker,
    STUDENT_CAREER_AI_TRACKER_DB: db,
    STUDENT_CAREER_AI_TRACKER_LOCK: lockDir,
    STUDENT_CAREER_AI_TRACKER_LOCK_RETRY_MS: '20',
  };
  const launchWriter = (timeoutMs) => {
    ...
    const child = spawn(NODE, [join(ROOT, script), ...resolvedArgs], {
      ...
      env: {
        ...childEnv,
        STUDENT_CAREER_AI_TRACKER_LOCK_TIMEOUT_MS: String(timeoutMs),
      },
...
// FOLLOW-UPS path and prefixed `student-career-ai-followups-` (followup-seed.mjs's
// FOLLOWUPS_LOCK_PREFIX and resolveLockDir) rather than the shared
// `student-career-ai-merge-tracker-` lock every writer above contends on.
//
...
async function testFollowupSeedUsesASeparateLockNamespace() {
  const dir = mkdtempSync(join(tmpdir(), 'student-career-ai-followup-seed-lock-'));
  const tracker = join(dir, 'applications.md');
  const followups = join(dir, 'follow-ups.md');
  const trackerLockDir = join(dir, 'student-career-ai-merge-tracker-followup-seed.lock');
  const followupsLockDir = join(dir, 'student-career-ai-followups-seed.lock');
  ...
  const child = spawn(NODE, [join(ROOT, 'followup-seed.mjs'), '1', '--json'], {
    ...
    env: {
      ...process.env,
      STUDENT_CAREER_AI_TRACKER: tracker,
      STUDENT_CAREER_AI_FOLLOWUPS: followups,
      STUDENT_CAREER_AI_FOLLOWUPS_LOCK: followupsLockDir,
      STUDENT_CAREER_AI_FOLLOWUPS_LOCK_RETRY_MS: '20',
      STUDENT_CAREER_AI_FOLLOWUPS_LOCK_TIMEOUT_MS: '3000',
      ...
      STUDENT_CAREER_AI_TRACKER_LOCK: trackerLockDir,
      STUDENT_CAREER_AI_TRACKER_LOCK_TIMEOUT_MS: '500',
      STUDENT_CAREER_AI_TRACKER_LOCK_RETRY_MS: '20',
    },
    stdio: ['pipe', 'pipe', 'pipe'],
  });
...
async function testTrackerLockReleaseRetriesPartialCleanup() {
  const dir = mkdtempSync(join(tmpdir(), 'student-career-ai-lock-release-'));
...
async function testTrackerLockReleasePreservesReplacementAfterPartialCleanup() {
  const dir = mkdtempSync(join(tmpdir(), 'student-career-ai-lock-replacement-'));
  const lockDir = join(dir, 'tracker.lock');
  let removeAttempts = 0;
  try {
    const lock = await acquireTrackerLock(lockDir, {
...
async function testTrackerTransactionCloseReportsCleanupFailure() {
  const dir = mkdtempSync(join(tmpdir(), 'student-career-ai-transaction-close-'));
  const tracker = join(dir, 'applications.md');
  const lockDir = join(dir, 'tracker.lock');
...
async function testReplyWatchConflictingRecommendations() {
  const dir = mkdtempSync(join(tmpdir(), 'student-career-ai-reply-conflict-'));
  const tracker = join(dir, 'applications.md');
  ...
  try {
    ...
    const child = spawn(NODE, [join(ROOT, 'reply-watch.mjs'), candidatesPath], {
      ...
      env: {
        ...
        STUDENT_CAREER_AI_TRACKER: tracker,
        STUDENT_CAREER_AI_TRACKER_DB: db,
        STUDENT_CAREER_AI_TRACKER_LOCK: join(dir, 'student-career-ai-merge-tracker-conflict.lock'),
        STUDENT_CAREER_AI_TRACKER_LOCK_TIMEOUT_MS: '1000',
        STUDENT_CAREER_AI_TRACKER_LOCK_RETRY_MS: '20',
      },
      stdio: ['pipe', 'pipe', 'pipe'],
    });
...
async function testFreshOwnerlessLockIsNotStolen() {
  const dir = mkdtempSync(join(tmpdir(), 'student-career-ai-ownerless-'));
  const lockDir = join(dir, 'tracker.lock');
  try {
...
async function testAgedOwnerlessLockStillRecovers() {
  const dir = mkdtempSync(join(tmpdir(), 'student-career-ai-ownerless-aged-'));
  const lockDir = join(dir, 'tracker.lock');
  try {
...
async function testLiveRecoverGuardIsNotEvicted() {
  const dir = mkdtempSync(join(tmpdir(), 'student-career-ai-guard-live-'));
  const lockDir = join(dir, 'tracker.lock');
