...
func NewViewerModel(t theme.Theme, studentCareerPath, path, title string, width, height int, app model.CareerApplication) ViewerModel {
	...
	m := ViewerModel{
		...
		coverLetterPath: parseCoverLetterPath(lines, studentCareerPath),
	}
	m.rebuildRender()
	return m
}
...
func parseCoverLetterPath(lines []string, studentCareerPath string) string {
	...
	for _, line := range lines {
		...
		if inCoverSection {
			if m := reCoverLetterPDF.FindStringSubmatch(line); m != nil {
				relPath := m[1]
				abs := filepath.Join(studentCareerPath, filepath.FromSlash(relPath))
	...
	case tea.KeyMsg:
		...
		case "L":
			if m.coverLetterPath != "" {
				fullPath := filepath.Join(m.studentCareerPath, filepath.FromSlash(m.coverLetterPath))
...
func (m ViewerModel) renderInlineElementsAs(line string, baseColor lipgloss.Color) string {
	...
	rest := line
	for rest != "" {
		match := findInlineMatch(rest, codeStyle, boldStyle, linkStyle, m.studentCareerPath)
		if match == nil {
...
func findInlineMatch(s string, codeStyle, boldStyle, linkStyle lipgloss.Style, studentCareerPath string) *inlineMatch {
	...
	if loc := reRelPDFPath.FindStringIndex(s); loc != nil {
		consider(loc, func() string {
			...
			if studentCareerPath == "" {
				return styled
			}
			joined := filepath.Join(studentCareerPath, filepath.FromSlash(relPath))
