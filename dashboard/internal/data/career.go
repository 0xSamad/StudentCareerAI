...
// relative to studentCareerPath. Links are normally relative to the tracker
...
func resolveReportPath(studentCareerPath, trackerPath, link string) string {
	resolved := filepath.Join(filepath.Dir(trackerPath), link)
	if _, err := os.Stat(resolved); err != nil {
		legacy := filepath.Join(studentCareerPath, link)
		if _, err2 := os.Stat(legacy); err2 == nil {
			resolved = legacy
		}
	}
	if rel, err := filepath.Rel(studentCareerPath, resolved); err == nil {
		return rel
	}
	return link
}
...
func ParseApplications(studentCareerPath string) []model.CareerApplication {
	filePath := filepath.Join(studentCareerPath, "applications.md")
	content, err := os.ReadFile(filePath)
	if err != nil {
		// Fallback: try data/ subdirectory
		filePath = filepath.Join(studentCareerPath, "data", "applications.md")
		content, err = os.ReadFile(filePath)
		if err != nil {
			return nil
		}
	...
	for _, line := range lines {
		...
		// back to a studentCareerPath-relative path, which is what every
		...
		if rm := reReportLink.FindStringSubmatch(at("report")); rm != nil {
			...
			app.ReportPath = resolveReportPath(studentCareerPath, filePath, rm[2])
		}
	...
	batchURLs := loadBatchInputURLs(studentCareerPath)
	reportNumURLs := loadJobURLs(studentCareerPath)

	for i := range apps {
		if apps[i].ReportPath == "" {
			continue
		}
		fullReport := filepath.Join(studentCareerPath, apps[i].ReportPath)
	...
	enrichFromScanHistory(studentCareerPath, apps)

	// Strategy 5: company name fallback from batch-input.tsv
	enrichAppURLsByCompany(studentCareerPath, apps)

	return apps
}

// loadBatchInputURLs reads batch-input.tsv and returns a map of batch ID -> job URL.
func loadBatchInputURLs(studentCareerPath string) map[string]string {
	inputPath := filepath.Join(studentCareerPath, "batch", "batch-input.tsv")
...
func loadJobURLs(studentCareerPath string) map[string]string {
	...
	statePath := filepath.Join(studentCareerPath, "batch", "batch-state.tsv")
...
func enrichFromScanHistory(studentCareerPath string, apps []model.CareerApplication) {
	scanPath := filepath.Join(studentCareerPath, "scan-history.tsv")
...
func enrichAppURLsByCompany(studentCareerPath string, apps []model.CareerApplication) {
	inputPath := filepath.Join(studentCareerPath, "batch", "batch-input.tsv")
	inputData, err := os.ReadFile(inputPath)
	if err != nil {
		return
...
// Aliases match states.yml -- keep in sync with student-career-ai/states.yml
...
func LoadReportSummary(studentCareerPath, reportPath string) (archetype, tldr, remote, comp string) {
	fullPath := filepath.Join(studentCareerPath, reportPath)
...
func UpdateApplicationStatus(studentCareerPath string, app model.CareerApplication, newStatus string) error {
	return UpdateApplicationStatusAndNotes(studentCareerPath, app, newStatus, "")
}
...
// notes unchanged.
func UpdateApplicationStatusAndNotes(studentCareerPath string, app model.CareerApplication, newStatus, notesAppend string) (returnErr error) {
	filePath := filepath.Join(studentCareerPath, "applications.md")
...
func LoadReportDiscardReasons(studentCareerPath, reportPath string) []string {
	if reportPath == "" {
		return nil
	}
	p := reportPath
	...
	fullPath := filepath.Join(studentCareerPath, p)
	content, err := os.ReadFile(fullPath)
	if err != nil {
		return nil
	}
...
func SaveAnonymousStat(studentCareerPath string, role string, weeks int) error {
	dirPath := filepath.Join(studentCareerPath, "data")
