...
const trackerLockPrefix = "student-career-ai-merge-tracker-"
...
func defaultTrackerLockOptions() trackerLockOptions {
	return trackerLockOptions{
		timeout: envMilliseconds("STUDENT_CAREER_AI_TRACKER_LOCK_TIMEOUT_MS", 60*time.Second),
		retry:   envMilliseconds("STUDENT_CAREER_AI_TRACKER_LOCK_RETRY_MS", 75*time.Millisecond),
		stale:   envMilliseconds("STUDENT_CAREER_AI_TRACKER_LOCK_STALE_MS", 10*time.Minute),
	}
}
...
func trackerLockDirFor(trackerPath string) (string, error) {
	...
	override := os.Getenv("STUDENT_CAREER_AI_TRACKER_LOCK")
