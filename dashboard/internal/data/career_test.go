...
func TestTrackerLockDirMatchesNodeProtocol(t *testing.T) {
	t.Setenv("STUDENT_CAREER_AI_TRACKER_LOCK", "")
	...
	want := filepath.Join(canonicalTemp, fmt.Sprintf("student-career-ai-merge-tracker-%x.lock", sum[:8]))
...
`
	...
	// Every consumer joins ReportPath against studentCareerPath — both rows
