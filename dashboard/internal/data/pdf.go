...
type PDFManifestEntry struct {
	ReportNumber string
	PDFPath      string // relative to the student-career-ai root
	HTMLPath     string // relative to the student-career-ai root, "" if unknown
...
func LoadPDFEntriesByPath(studentCareerPath string) map[string]PDFManifestEntry {
	byPath := make(map[string]PDFManifestEntry)
	raw, err := os.ReadFile(filepath.Join(studentCareerPath, "data", "pdf-index.tsv"))
	if err != nil {
		return byPath
	}
...
// LoadPDFManifest reads data/pdf-index.tsv under studentCareerPath. A missing
...
func LoadPDFManifest(studentCareerPath string) PDFManifest {
...
// ResolvePDFs returns candidate PDF paths (relative to studentCareerPath) for an
...
func ResolvePDFs(studentCareerPath string, app model.CareerApplication, manifest PDFManifest) []string {
	if entry, ok := manifest.Lookup(app); ok {
		if _, err := os.Stat(filepath.Join(studentCareerPath, filepath.FromSlash(entry.PDFPath))); err == nil {
	...
	globbed, err := filepath.Glob(filepath.Join(studentCareerPath, "output", "cv-*.pdf"))
	if err != nil {
		return nil
	}
	...
	for _, p := range globbed {
		...
		if matchesCompanySlug(base, slug) {
			if rel, err := filepath.Rel(studentCareerPath, p); err == nil {
				matches = append(matches, filepath.ToSlash(rel))
			}
		}
	}

	sortPDFsNewestFirst(studentCareerPath, matches)
	return matches
}
...
func sortPDFsNewestFirst(studentCareerPath string, paths []string) {
	...
	mtimeOf := func(p string) int64 {
		info, err := os.Stat(filepath.Join(studentCareerPath, filepath.FromSlash(p)))
		if err != nil {
			return 0
		}
...
func ResolveHTML(studentCareerPath string, app model.CareerApplication) (htmlPath, pdfPath string) {
	slug := kebabCase(app.Company)
	if slug == "" {
		return "", ""
	}
	globbed, err := filepath.Glob(filepath.Join(studentCareerPath, "output", "cv-*.html"))
	if err != nil {
		return "", ""
	}
	var matches []string
