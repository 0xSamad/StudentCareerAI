...
import (
	...
	"github.com/0xSamad/StudentCareerAI/dashboard/internal/ui/screens"
)
...
func (m *appModel) reloadPipelineData() {
	apps := data.ParseApplications(m.studentCareerPath)
...
func (m appModel) Update(msg tea.Msg) (tea.Model, tea.Cmd) {
	...
	case screens.PipelineLoadReportMsg:
		archetype, tldr, remote, comp := data.LoadReportSummary(msg.StudentCareerPath, msg.ReportPath)
		m.pipeline.EnrichReport(msg.ReportPath, archetype, tldr, remote, comp)
		return m, nil

	case screens.PipelineUpdateStatusMsg:
		err := data.UpdateApplicationStatus(msg.StudentCareerPath, msg.App, msg.NewStatus)
		if err != nil {
	...
	case screens.PipelineUpdateStatusAndNotesMsg:
		...
		err := data.UpdateApplicationStatusAndNotes(msg.StudentCareerPath, msg.App, msg.NewStatus, msg.NotesAppend)
		if err != nil {
	...
	case screens.ViewerUpdateStatusMsg:
		...
		if normalized == "hired" {
			err := data.UpdateApplicationStatus(m.studentCareerPath, msg.App, msg.NewStatus)
			if err != nil {
				fmt.Fprintf(os.Stderr, "WARN: status update failed: %v\n", err)
				m.reloadPipelineData()
				return m, nil
...
// runGeneratePDF shells out to node generate-pdf.mjs in the student-career-ai root,
...
func runGeneratePDF(msg screens.PipelineGeneratePDFMsg) tea.Cmd {
	return func() tea.Msg {
		...
		cmd.Dir = msg.StudentCareerPath
		out, err := cmd.CombinedOutput()
		if err != nil {
			return screens.PipelinePDFGeneratedMsg{Err: summarizeCmdError(err, out)}
		}
		pdfAbs := filepath.Join(msg.StudentCareerPath, filepath.FromSlash(msg.PDFPath))
...
func main() {
	pathFlag := flag.String("path", ".", "Path to student-career-ai directory")
	...
	studentCareerPath := *pathFlag

	// Load applications
	apps := data.ParseApplications(studentCareerPath)
	if apps == nil {
	...
	pm := screens.NewPipelineModel(t, apps, metrics, studentCareerPath, 120, 40)

	for _, app := range apps {
		if app.ReportPath == "" {
			continue
		}
		archetype, tldr, remote, comp := data.LoadReportSummary(studentCareerPath, app.ReportPath)
