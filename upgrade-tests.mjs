...
const CANONICAL = 'https://github.com/0xSamad/StudentCareerAI.git';
const TAG_RE = /^student-career-ai-v(\d+)\.(\d+)\.(\d+)$/;
...
export function releaseTags(cwd = ROOT) {
  return git(cwd, 'tag', '--list', 'student-career-ai-v*').split('\n').filter((t) => TAG_RE.test(t))
...
function writeGitConfig(work, mirror) {
  ...
  writeFileSync(cfg, `[user]\n\tname = upgrade-tests\n\temail = upgrade-tests@student-career-ai.test\n[url "${url}"]\n\tinsteadOf = ${CANONICAL}\n[safe]\n\tdirectory = *\n`);
  return cfg;
}
