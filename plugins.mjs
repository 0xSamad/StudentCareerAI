...
async function cmdRun(args) {
  ...
  if (hook === 'ingest' || hook === 'search') {
    ...
    if (jobs.length) { await appendToPipeline(jobs); console.log(`→ Appended ${jobs.length} to data/pipeline.md. Run /student-career-ai pipeline to evaluate.`); }
    return;
  }
  ...
  if (hook === 'notify') {
    const message = positional.slice(hookArgStart).join(' ') || '(student-career-ai notification)';
...
function setEnabled(id, on, settings) {
  ...
  writeFileSync(file, '# student-career-ai plugin activation — see config/plugins.example.yml\n' + yaml.dump(cfg), 'utf8');
}
...
async function cmdAvailable() {
  ...
  console.log('\n✓ Community plugins approved by student-career-ai:\n');
  if (reg.plugins.length === 0) {
    console.log('  (none yet — publish yours as `student-career-ai-plugin-<name>` and open a registry PR; see docs/PLUGINS.md)');
  } else {
...
function cmdEnable(args) {
  ...
  if (!confirm) {
    ...
    if (source === 'unverified') console.log('\n  ⚠️  UNVERIFIED — not reviewed by student-career-ai; you are trusting this author.');
...
function cmdNew(args) {
  ...
  console.log(`    B) publish:          push a github repo named "student-career-ai-plugin-${name}", then open a registry PR (docs/PLUGINS.md)`);
}
...
async function cmdAdd(args) {
  ...
  if (reg && !target.includes('/')) {
    ...
    console.log("⚠️  Not in the student-career-ai registry — you are installing this author's code with your privileges.");
  }
