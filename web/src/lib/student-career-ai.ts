...
export function rootScript(nameNoExt: string): string {
  return path.join(studentCareerRoot(), `${nameNoExt}.mjs`);
}
...
function read(rel: string): string | null {
  try {
    return fs.readFileSync(path.join(studentCareerRoot(), rel), "utf8");
  } catch {
    return null;
  }
}
...
export function readApplications(): Application[] {
  ...
  if (!md) return [];
  return parseApplications(md, studentCareerRoot());
}
...
export function doctorState(): {
  ...
  const has = (rel: string) => {
    try {
      return fs.existsSync(path.join(studentCareerRoot(), rel));
    } catch {
      return false;
    }
  };
...
export function profilePath(): string {
  return path.join(studentCareerRoot(), "modes", "_profile.md");
}
