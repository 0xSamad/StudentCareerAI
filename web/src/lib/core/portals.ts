...
export function writeTempPortals(f: FilterLists): string {
  const file = path.join(os.tmpdir(), `student-career-ai-explore-${randomUUID()}.yml`);
...
export function cleanupTempPortals(file: string): void {
  try {
    if (file.startsWith(os.tmpdir()) && file.includes("student-career-ai-explore-")) fs.unlinkSync(file);
  } catch {
    /* best-effort */
  }
}
...
function loadYaml(rel: string): Record<string, unknown> | null {
  try {
    const doc = yaml.load(fs.readFileSync(path.join(studentCareerRoot(), rel), "utf8"));
