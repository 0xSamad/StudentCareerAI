...
import { studentCareerRoot, readInbox, readApplications } from "@/lib/student-career-ai";
...
export function assembleDedupContext(): { urls: Set<string>; lines: string[] } {
  ...
  try {
    const tsv = fs.readFileSync(path.join(studentCareerRoot(), "data", "scan-history.tsv"), "utf8");
