...
export async function fetchGreenhouseSchema(token: string, jobId: string): Promise<Map<string, GhField> | null> {
  try {
    ...
    const data = (await r.json()) as {
      questions?: Array<{ label?: string; required?: boolean; fields?: Array<{ name?: string; type?: string; values?: Array<{ label?: string }> }> }>;
      location_questions?: Array<{ label?: string; required?: boolean; fields?: Array<{ name?: string; type?: string; values?: Array<{ label?: string }> }> }>;
    };
    const map = new Map<string, GhField>();
    const questions = [...(data.questions ?? []), ...(data.location_questions ?? [])];
    for (const q of questions) {
      ...
      for (const f of q.fields ?? []) {
        ...
        map.set(f.name, gf);
        map.set(f.name.replace(/\[\]$/, ""), gf);
        if (label) {
          map.set(`label:${label.toLowerCase()}`, gf);
          map.set(`label:${label.toLowerCase().replace(/\s*\*+\s*$/, "")}`, gf);
        }
      }
    }
