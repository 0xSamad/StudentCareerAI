...
import type { InboxJob } from "@/lib/student-career-ai";
...
export function InboxTriage({ inbox }: { inbox: InboxJob[] }) {
  ...
  useEffect(() => {
    try {
      const s = readJsonKey(SHORTLIST_KEY, LEGACY_SHORTLIST);
      if (s) setShortlist(s);
      const h = readJsonKey(HIDDEN_KEY, LEGACY_HIDDEN);
      if (h) setHidden(h);
      const c = readJsonKey(CONFIG_KEY, LEGACY_CONFIG);
      setHasCli(!!(c && c.cliId));
    } catch {
      /* ignore */
    }
