...
import type { Application, InboxJob } from "@/lib/student-career-ai";
...
export function ExplorerView({
  ...
  useEffect(() => {
    try {
      const id = JSON.parse(localStorage.getItem("student-career-ai:config") || "{}").cliId || null;
    ...
    <div className="mx-auto max-w-5xl px-5 py-8 md:px-8">
      ...
      {!rootExists && (
        <div className="mb-5 rounded-lg border border-amber-500/30 bg-amber-500/10 px-4 py-3 text-sm text-amber-700 dark:text-amber-300">
          Your StudentCareer AI home isn’t set up yet — discovery needs a checkout with a profile to seed from.
        </div>
      )}
...
function FailedCard({ msg, onRetry }: { msg: string; onRetry: () => void }) {
  ...
  const scannerMissing = /isn'?t available|data only|complete student-career-ai checkout|scanner/i.test(msg);
  if (scannerMissing) {
    return (
      <div className="rounded-2xl border border-border bg-surface/30 px-6 py-10 text-center">
        ...
        <p className="mx-auto mt-1.5 max-w-md text-sm text-muted">
          Your StudentCareer AI home looks data-only or is on an older version. The free scanner ships with a complete checkout —
          update the project, or paste a job URL on the pipeline to evaluate it directly.
        </p>
