...
// page AND written to <root>/.student-career-ai-web/apply-prefill.log for debugging.
export async function POST(req: Request) {
  ...
  const logPath = path.join(studentCareerRoot(), ".student-career-ai-web", "apply-prefill.log");
  try {
...
Output ONLY a compact JSON object mapping each field id → {"value": "...", "needs_confirmation": boolean}. No prose, no markdown, no code fence.`;
      ...
      const result = await new Promise<{ buf: string; code: number | null; signal: NodeJS.Signals | null }>((resolve) => {
        ...
        const child = spawn(binPath, args, { cwd: studentCareerRoot(), env: process.env, stdio: ["ignore", "pipe", "pipe"] });
        let buf = "";
        let firstByteAt = 0;
