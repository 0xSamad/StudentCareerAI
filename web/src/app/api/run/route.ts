...
import { studentCareerRoot, readMemory, findReportFile } from "@/lib/student-career-ai";
...
export async function POST(req: Request) {
  ...
  if ((kind === "evaluate" || kind === "pdf") && !fs.existsSync(path.join(studentCareerRoot(), "cv.md"))) {
    return new Response(
  ...
  if (kind === "pdf") {
    const pathsResult = resolvePdfPaths(input, today, studentCareerRoot(), findReportFile);
    if (!pathsResult.ok) {
  ...
  const reportsDir = path.join(studentCareerRoot(), "reports");
  const countReports = () => {
    try {
