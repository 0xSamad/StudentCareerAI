...
import { studentCareerRoot, rootScript, trackerCanDelete } from "@/lib/student-career-ai";
...
export async function POST(req: Request) {
  ...
  if (!trackerCanDelete()) {
    return Response.json(
      { error: "Removing a tracker row needs a newer student-career-ai — update to delete rows from here." },
      { status: 400 },
    );
  }
  ...
  try {
    const result = await new Promise<{ code: number | null; err: string }>((resolve) => {
      ...
      let child;
      try {
        child = spawn(process.execPath, args, { cwd: studentCareerRoot(), env: process.env });
      } catch (e) {
