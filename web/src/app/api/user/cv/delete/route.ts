...
export async function POST(req: Request) {
  try {
    ...
    if (!userId) {
      return NextResponse.json({ error: "userId is required for CV deletion" }, { status: 400 });
    }

    const result = await container.dataPrivacyService.deleteCV({
      tenantId,
      userId,
      role: "user",
    });
    ...
    return NextResponse.json(
      { error: err.message || "Failed to delete CV" },
