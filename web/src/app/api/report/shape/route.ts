...
import { studentCareerRoot, doctorState, readApplications, readInbox, trackerCanDelete } from "@/lib/student-career-ai";
...
function lineCount(rel: string, predicate: (l: string) => boolean): number {
  try {
    return fs
      .readFileSync(path.join(studentCareerRoot(), rel), "utf8")
      .split("\n")
...
function dirCount(rel: string, ext: string): number {
  try {
    return fs.readdirSync(path.join(studentCareerRoot(), rel)).filter((f) => f.endsWith(ext)).length;
  } catch {
    return 0;
  }
}
...
export async function GET() {
  ...
  return Response.json({
    ...
    data: {
      ...
      followupsFile: fs.existsSync(path.join(studentCareerRoot(), "data", "follow-ups.md")),
    },
    capabilities: {
