...
function readCanonicalMode(): string | null {
  try {
    return fs.readFileSync(path.join(studentCareerRoot(), "modes", "cv-ingest.md"), "utf8");
  } catch {
    return null;
  }
}
...
function ingestPrompt(source: string): string {
  ...
  return `You convert a person's CV into clean cv.md markdown that EXACTLY mirrors student-career-ai's reference format.
...
export async function POST(req: Request) {
  ...
  try {
    if (ctype.includes("application/json")) {
      ...
      const dir = fs.mkdtempSync(path.join(os.tmpdir(), "student-career-ai-cv-"));
  ...
  let child;
  try {
    child = spawn(binPath, args, { cwd: studentCareerRoot(), env: process.env });
  } catch (e) {
