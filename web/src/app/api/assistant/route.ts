...
import { studentCareerRoot, readMemory, doctorState } from "@/lib/student-career-ai";

export const runtime = "nodejs"; // child_process (spawn) requires the Node runtime
export const dynamic = "force-dynamic";
export const maxDuration = 120;

const SYSTEM_PREAMBLE = `You are the StudentCareer AI assistant — a proactive, friendly career co-pilot for a student who is actively job-hunting. You live inside their LOCAL StudentCareer AI web dashboard (internships, jobs, tailored CVs, URL apply, analytics) and run on their own AI CLI.

YOUR MISSION: genuinely help THIS person land a great role. Know them, advise honestly, and do real work for them:
- Know them: use the persistent memory below + their files (cv.md, config/profile.yml, reports/, data/applications.md, and past worker logs in .student-career-ai-web/runs/{id}.md). Read them to be concrete.
...
export async function POST(req: Request) {
  ...
  const child = spawn(binPath, args, { cwd: studentCareerRoot(), env: process.env });
