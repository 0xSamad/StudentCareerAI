...
import { Briefcase, Sparkles, Search, RefreshCw } from "lucide-react";
...
export default function JobsPage() {
  ...
  const fetchJobs = (opts?: { silent?: boolean }) => {
    if (!opts?.silent) setLoading(true);
    const query = new URLSearchParams({ type: "JOB", limit: "400", verifiedOnly: "false" });
    ...
    try {
      const { ok, data } = await runOpportunityScan(
        ...
        },
        {
          ...
          onPartial: () => fetchJobs({ silent: true }),
        }
      );
  ...
  useEffect(() => {
    fetchJobs();
    const onScan = () => fetchJobs({ silent: true });
    ...
    (async () => {
      ...
      const result = await watchOpportunityScan({
        ...
        onPartial: () => {
          if (!cancelled) fetchJobs({ silent: true });
        },
      });
      if (cancelled) return;
      fetchJobs({ silent: true });
  ...
  return (
    <div className="mx-auto max-w-6xl px-4 sm:px-6 py-8 space-y-8 max-sm:pb-24">
      <PageHeader
        ...
        title="Graduate & full-time jobs"
        description="Pakistan jobs first, then remote. Refresh scans the Pakistan Top 100, international remote roles, and Adzuna. Apply opens the job form URL — not a career homepage."
        actions={
          <button onClick={() => fetchJobs()} className={buttonSecondaryClassName}>
      ...
      {scanMessage && (
        <div className="p-4 bg-brand/10 border border-brand/30 rounded-xl flex items-center gap-3 text-brand-text text-sm">
      ...
      <AddToQueueToolbar opportunities={opportunities} selectedIds={selectedIds} onAdded={fetchQueued} />
      ...
      <div className="flex flex-wrap items-center gap-3">
        ...
        <div className="relative flex-1 min-w-[240px]">
          ...
          <input
            type="text"
            placeholder="Search full-time roles by company, role, or stack..."
            value={searchQuery}
      ...
      {loading ? (
        <div className="flex flex-col items-center justify-center py-20 text-muted">
          ...
          <p className="text-xs">Loading saved jobs…</p>
        </div>
      ) : opportunities.length === 0 ? (
        <div className="rounded-2xl border border-dashed border-border bg-surface/40 p-12 text-center space-y-3">
          <Briefcase className="size-10 text-faint mx-auto" />
          <h3 className="font-bold text-base text-foreground">No full-time roles currently in feed</h3>
