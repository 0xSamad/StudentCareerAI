 ...
 * *acting on* it.
 *
 * student-career-ai is driven from an AI session, but there's no durable place to drop
...
const PATH = process.env.STUDENT_CAREER_AI_INBOX || 'data/agent-inbox.md';

const HEADER = [
  '# Agent Inbox',
  '',
  '> **Agent protocol:** at the start of a student-career-ai session, read this file.',
...
function ensureGitignored() {
  ...
  if (process.env.STUDENT_CAREER_AI_INBOX || PATH !== 'data/agent-inbox.md') return;
  try {
