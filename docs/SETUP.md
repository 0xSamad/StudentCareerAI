...
`npx` ships with Node.js — it runs the installer once without installing anything globally. This clones the latest release into `./student-career-ai` and installs dependencies. Then move into the workspace and open your AI CLI:
...
**On first launch, student-career-ai walks you through setup by chatting** — it asks for your CV, your details (name, target roles, salary), and sets up the job scanner with pre-configured companies. Nothing to edit by hand: just answer its questions. Then paste a job offer URL or description and it evaluates it, writes a report, generates a tailored PDF, and tracks it.

If you are using Codex, start the interactive session with `codex`. Slash commands are not guaranteed in Codex, so use the same mode names in a prompt if `/student-career-ai` is unavailable:

```text
Evaluate this JD with student-career-ai auto-pipeline: https://company.com/jobs/123
Run the student-career-ai scan mode.
Run the student-career-ai pipeline mode.
Run the student-career-ai pdf mode.
Run the student-career-ai email mode for the latest evaluated role. Draft only; never sends, submits, or clicks.
Run the student-career-ai tracker mode.
```
...
codex exec "Run student-career-ai scan mode in this repo."
...
| Search for offers | `/student-career-ai scan` or ask the agent to run `scan` |
| Process pending URLs | `/student-career-ai pipeline` or ask the agent to run `pipeline` |
| Generate a PDF | `/student-career-ai pdf` or ask the agent to run `pdf` |
| Draft application email | `/student-career-ai email` or ask the agent to run `email`; draft-only, never sends, submits, or clicks |
| Batch evaluate | `/student-career-ai batch` or use `codex exec "Run student-career-ai batch mode ..."` |
| Check tracker status | `/student-career-ai tracker` or ask the agent to run `tracker` |
| Fill application form | `/student-career-ai apply` or ask the agent to run `apply` |
