...
`student-career-ai` offers to scan for you on a schedule ("just say *scan every 3 days*"),
...
> local files. Evaluating a shortlisted role later (`/student-career-ai pipeline`) is the
...
Replace `/path/to/student-career-ai` with your checkout path, and make sure `node` is on
...
```cron
0 9 */3 * * cd /path/to/student-career-ai && /usr/local/bin/node scan.mjs >> data/scan.log 2>&1
```
...
```cron
0 8 * * 1-5 cd /path/to/student-career-ai && /usr/local/bin/node scan.mjs >> data/scan.log 2>&1
```

### macOS — launchd (survives sleep better than cron)

Save as `~/Library/LaunchAgents/io.student-career-ai.scan.plist`, then
`launchctl load ~/Library/LaunchAgents/io.student-career-ai.scan.plist`:
...
<plist version="1.0">
<dict>
  <key>Label</key>            <string>io.student-career-ai.scan</string>
  <key>ProgramArguments</key>
  <array>
    <string>/usr/local/bin/node</string>
    <string>scan.mjs</string>
  </array>
  <key>WorkingDirectory</key> <string>/path/to/student-career-ai</string>
  <key>StartCalendarInterval</key>
  <dict>
    <key>Hour</key>    <integer>9</integer>
    <key>Minute</key>  <integer>0</integer>
  </dict>
  <key>StandardOutPath</key>   <string>/path/to/student-career-ai/data/scan.log</string>
  <key>StandardErrorPath</key> <string>/path/to/student-career-ai/data/scan.log</string>
</dict>
</plist>
```
...
```powershell
$action  = New-ScheduledTaskAction -Execute "node.exe" -Argument "scan.mjs" -WorkingDirectory "C:\path\to\student-career-ai"
$trigger = New-ScheduledTaskTrigger -Daily -At 9am
Register-ScheduledTask -TaskName "student-career-ai scan" -Action $action -Trigger $trigger -Description "Recurring student-career-ai job scan"
```
...
## How this fits the rest of student-career-ai
