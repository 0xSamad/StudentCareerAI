...
Plugins extend student-career-ai with integrations that need an API key or talk to an
...
> extend student-career-ai itself.
...
If a plugin's files change without a version bump, student-career-ai **blocks it** and
...
1. Develop locally, then publish your plugin as its **own public GitHub repo**
   named exactly `student-career-ai-plugin-<name>` (the template repo gives you the
...
1. Publish `student-career-ai-plugin-<id>` with the **same `id`** as the bundled plugin
...
3. Once approved + pinned, anyone who runs `node plugins.mjs add student-career-ai-plugin-<id>`
   ...
   surfaces the link: *"gmail — 🔁 maintained version: student-career-ai-plugin-gmail"*.
...
code on your own machine — student-career-ai is local-first and the source is yours; if
...
| --- | --- | --- | --- | --- |
| [student-career-ai-plugin-tavily](https://github.com/Schlaflied/student-career-ai-plugin-tavily) | Tavily search/extract for job scanning, liveness checks, and company research. | search | `TAVILY_API_KEY` | @Schlaflied |
| [student-career-ai-plugin-google-calendar](https://github.com/Schlaflied/student-career-ai-plugin-google-calendar) | Google Calendar ingest — detect upcoming interview events and surface them in the student-career-ai pipeline. | ingest | `GOOGLE_CALENDAR_CLIENT_ID`, `GOOGLE_CALENDAR_CLIENT_SECRET`, `GOOGLE_CALENDAR_REFRESH_TOKEN` | @Schlaflied |
| [student-career-ai-plugin-linkedin-alerts](https://github.com/Schlaflied/student-career-ai-plugin-linkedin-alerts) | LinkedIn job alert ingest — parse LinkedIn alert emails from your Gmail inbox, normalize tracking links to canonical job URLs, and surface them in the student-career-ai pipeline. | ingest | `GMAIL_CLIENT_ID`, `GMAIL_CLIENT_SECRET`, `GMAIL_REFRESH_TOKEN` | @Schlaflied |
| [student-career-ai-plugin-outlook-interviews](https://github.com/Schlaflied/student-career-ai-plugin-outlook-interviews) | Outlook interview ingest — detect interview invitation emails via Microsoft Graph, extract company / role / meeting link, and surface them in the student-career-ai pipeline. | ingest | `MSGRAPH_CLIENT_ID`, `MSGRAPH_REFRESH_TOKEN` (optional: `MSGRAPH_CLIENT_SECRET`) | @Schlaflied |
| [student-career-ai-plugin-obsidian](https://github.com/Schlaflied/student-career-ai-plugin-obsidian) | Obsidian export — mirror the tracker into your vault as frontmatter notes queryable by Dataview/Bases; frontmatter belongs to the machine, the note body belongs to you. | export | None | @Schlaflied |
...
- **Centralized infrastructure** the project would run (hosted aggregation,
  ...
  [Discussion #904](https://github.com/0xSamad/StudentCareerAI/discussions/904).
- **Auto-submitting / blind-applying** → out of the core everywhere. student-career-ai
