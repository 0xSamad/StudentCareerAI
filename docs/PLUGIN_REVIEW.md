...
- **Listed** — one `plugins-registry/<id>.json` file; the code stays in the
  author's `student-career-ai-plugin-<name>` repo; users `add` it. The default, low
  burden.
...
- **Naming + identity:** repo is `student-career-ai-plugin-<name>`, `id` == name minus
...
- **Wording (public-forever):** description / README / skill carry no commercial
  / hosted-service / monetization language. student-career-ai is free and local-first;
...
**not** registry-listed. It can still be a `student-career-ai-plugin-<name>` repo users
...
feature PRs against it — close-redirect them to "publish `student-career-ai-plugin-<id>`
