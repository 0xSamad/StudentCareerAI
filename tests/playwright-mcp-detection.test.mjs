...
try {
  ...
  // 5. STUDENT_CAREER_AI_CLI=opencode via env (no flag, no .env), no opencode.json.
  {
    ...
    try {
      ...
      if (!expectWarn(state, '#5 env=opencode')) {
        // already failed
          ...
          && Array.isArray(state.warnings)
          && state.warnings.some((w) => PLAYWRIGHT_RE.test(w) && /active cli: opencode/i.test(w))) {
        pass('STUDENT_CAREER_AI_CLI env + no config → warning, cli_source=env');
      } else {
  ...
  // 6. .env in --target has STUDENT_CAREER_AI_CLI=opencode, process.env unset.
  {
    const dir = mkdtempSync(join(tmpdir(), 'co-mcp-6-'));
    try {
      writeFileSync(join(dir, '.env'), 'STUDENT_CAREER_AI_CLI=opencode\n');
      ...
      if (!expectWarn(state, '#6 .env=opencode')) {
        // already failed
          ...
          && Array.isArray(state.warnings)
          && state.warnings.some((w) => PLAYWRIGHT_RE.test(w))) {
        pass('.env file with STUDENT_CAREER_AI_CLI → cli_source=.env, warning fires');
      } else {
  ...
  // 11. STUDENT_CAREER_AI_CLI=vim (invalid env) → same as #10 but via env path.
  //     active_cli='unknown', cli_source='env', one warning with the env path.
  {
    const dir = mkdtempSync(join(tmpdir(), 'co-mcp-11-'));
    try {
      const state = runDoctor(dir, [], { STUDENT_CAREER_AI_CLI: 'vim' });
      if (state._error) { fail(`#11 doctor crashed: ${state._error}`); }
          ...
          && Array.isArray(state.warnings)
          && state.warnings.length === 1
          && /STUDENT_CAREER_AI_CLI="vim"/.test(state.warnings[0])) {
        pass('STUDENT_CAREER_AI_CLI="vim" (invalid) → unknown sentinel, one warning via env path');
      } else {
