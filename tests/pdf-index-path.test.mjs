...
// own install directory while resolving the tracker through STUDENT_CAREER_AI_TRACKER.
...
// is restored — a leaked STUDENT_CAREER_AI_PDF_INDEX would silently retarget later
// suites (PIT: each case must stand alone, in any order).
function withPdfIndexEnv(value, body) {
  const previous = process.env.STUDENT_CAREER_AI_PDF_INDEX;
  if (value === undefined) delete process.env.STUDENT_CAREER_AI_PDF_INDEX;
  else process.env.STUDENT_CAREER_AI_PDF_INDEX = value;
  try {
    return body();
  } finally {
    if (previous === undefined) delete process.env.STUDENT_CAREER_AI_PDF_INDEX;
    else process.env.STUDENT_CAREER_AI_PDF_INDEX = previous;
  }
}
...
// Given STUDENT_CAREER_AI_PDF_INDEX, When resolving, Then the override wins outright.
withPdfIndexEnv('/elsewhere/custom-index.tsv', () => {
  const resolved = resolvePdfIndexPath(join('/ws', 'data', 'applications.md'));
  if (resolved === '/elsewhere/custom-index.tsv') pass('STUDENT_CAREER_AI_PDF_INDEX overrides the derived path');
...
// workspace's own manifest — never the student-career-ai install directory's, which is
...
withPdfIndexEnv(undefined, () => {
  const isolatedWorkspace = join('/tmp', 'student-career-ai-fixture');
