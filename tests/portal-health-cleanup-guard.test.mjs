...
const dir = mkdtempSync(join(tmpdir(), 'student-career-ai-guard-'));
...
try {
  ...
  //    range instead of waiting out the lock's real default timeout.
  {
    ...
    writeFileSync(p, HEADER + '2026-01-01\t' + marker + '\treachable\n', 'utf-8');
    const prevTimeout = process.env.STUDENT_CAREER_AI_PORTAL_HEALTH_LOCK_TIMEOUT_MS;
    const prevRetry = process.env.STUDENT_CAREER_AI_PORTAL_HEALTH_LOCK_RETRY_MS;
    process.env.STUDENT_CAREER_AI_PORTAL_HEALTH_LOCK_TIMEOUT_MS = '200';
    process.env.STUDENT_CAREER_AI_PORTAL_HEALTH_LOCK_RETRY_MS = '20';
    ...
    try {
      ...
      held.release();
      if (prevTimeout === undefined) delete process.env.STUDENT_CAREER_AI_PORTAL_HEALTH_LOCK_TIMEOUT_MS;
      else process.env.STUDENT_CAREER_AI_PORTAL_HEALTH_LOCK_TIMEOUT_MS = prevTimeout;
      if (prevRetry === undefined) delete process.env.STUDENT_CAREER_AI_PORTAL_HEALTH_LOCK_RETRY_MS;
      else process.env.STUDENT_CAREER_AI_PORTAL_HEALTH_LOCK_RETRY_MS = prevRetry;
    }
  }
} finally {
