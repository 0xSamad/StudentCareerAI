...
try {
  ...
  const origModelEnv = process.env.STUDENT_CAREER_AI_MODEL;
  delete process.env.STUDENT_CAREER_AI_MODEL;
  const freeCost = estimateCost('meta-llama/llama-3.1-70b-instruct:free', usage, 'openrouter');
  if (origModelEnv !== undefined) {
    process.env.STUDENT_CAREER_AI_MODEL = origModelEnv;
  }
  if (freeCost === 0) {
    pass('OpenRouter :free / free-rotation exemption returns 0 cost when no STUDENT_CAREER_AI_MODEL is pinned');
  } else {
