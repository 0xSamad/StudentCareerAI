...
// tracker-columns-tests.mjs: a throwaway tracker via the STUDENT_CAREER_AI_TRACKER /
// STUDENT_CAREER_AI_TRACKER_LOCK env overrides tracker-utils.mjs already respects.
//
...
function makeSandbox(trackerContent) {
  ...
  // Must live under tmpdir and use the student-career-ai lock-name prefix (see
  // trackerLockDirFor) or it's ignored — still safe, just a shared-lock risk.
  const lock = join(dir, 'student-career-ai-merge-tracker-test.lock');
...
function runMarkPdfReady(args, sandbox, extraEnv = {}) {
  const env = {
    ...process.env,
    STUDENT_CAREER_AI_TRACKER: sandbox.tracker,
    STUDENT_CAREER_AI_TRACKER_LOCK: sandbox.lock,
    ...extraEnv,
  };
  try {
...
{
  // Given STUDENT_CAREER_AI_TRACKER points at a path with no tracker
  const dir = mkdtempSync(join(tmpdir(), 'co-markpdf-missing-'));
  const sandbox = { tracker: join(dir, 'does-not-exist.md'), lock: join(dir, 'student-career-ai-merge-tracker-test.lock') };
  try {
...
// ── 12. Lock timeout: structured exit 4, no write ──
{
  ...
  try {
    ...
    const r = runMarkPdfReady(['1', '--json'], sb, { STUDENT_CAREER_AI_TRACKER_LOCK_TIMEOUT_MS: '300' });
    let parsed = null;
    try { parsed = JSON.parse(r.stdout); } catch { /* asserted below */ }
...
// ── 13. Non-timeout lock failure -> exit 1, not lock-timeout ──
{
  ...
  const blocker = join(dir, 'student-career-ai-merge-tracker-blocker');
  writeFileSync(blocker, 'not a directory');
  const badLock = join(blocker, 'student-career-ai-merge-tracker-bad.lock');
  try {
...
// ── 14. Write failure surfaces as a structured error, not a stack ──
{
  if (process.platform !== 'win32' && process.getuid?.() === 0) {
    ...
    writeFileSync(tracker, TRACKER_9);
    const lock = join(dir, 'student-career-ai-merge-tracker-wf.lock');
