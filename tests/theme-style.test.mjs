...
try {
  ...
  const dir = mkdtempSync(join(tmpdir(), 'student-career-ai-theme-'));
  try {
  ...
  if (block.includes('id="student-career-ai-dynamic-theme"') && block.includes(':root {') && block.includes('--accent-color: #2563eb;') && block.includes('--font-size: 10pt;')) {
  ...
  if (injected.includes('student-career-ai-dynamic-theme') && injected.indexOf('student-career-ai-dynamic-theme') < injected.indexOf('</head>') && injected.indexOf('student-career-ai-dynamic-theme') > injected.indexOf('<style>')) {
  ...
  if (noHead.startsWith('<style id="student-career-ai-dynamic-theme"')) pass('injectThemeStyle prepends the block when there is no </head>');
  ...
  // a --page-margin override earlier in <head> is what actually wins.
  {
    ...
    const overrideIdx = withOverride.indexOf('student-career-ai-dynamic-theme'); // the profile's style.margin override
    const pageSetupIdx = withOverride.indexOf('student-career-ai-page-setup');   // injectPrintPageCss's @page rule
