...
try {
  ...
  const urlsOk = calls.length === 2
    ...
    && calls.every((u) => new URL(u).searchParams.get('source') === 'student-career-ai')
  ...
  if (urlsOk && jobs.length === 52 && jobs[0].title === 'Role 0' && jobs[51].title === 'Role 51') {
    pass('fetch() paginates 0-based with source=student-career-ai + q passthrough, stops after total_pages, drops invalid rows');
  } else {
