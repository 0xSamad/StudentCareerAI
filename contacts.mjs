 ...
 * UID (studentcareer-{uidPart(name)}--{uidPart(company)}, where each part is
...
const CONTACTS_PATH = join(STUDENT_CAREER_AI, 'data/contacts.tsv');
const DEFAULT_VCF = join(STUDENT_CAREER_AI, 'output/contacts.vcf');
...
export function contactUid(c) {
  return `studentcareer-${uidPart(c.name)}--${uidPart(c.company)}`;
}
...
export function contactToVcard(contact, { callerId = false, rev = null } = {}) {
  ...
  lines.push('CATEGORIES:student-career-ai');
...
function selfTest() {
  ...
  assert(quality.duplicates.length === 1 && /^studentcareer-jane-doe-[0-9a-f]{8}--acme-[0-9a-f]{8}$/.test(quality.duplicates[0].uid) && quality.duplicates[0].count === 2,
  ...
  assert(/UID:studentcareer-[0-9a-f]{8}--globex-[0-9a-f]{8}\r\n/.test(cjkCard), 'CJK contact UID: bare hash name part, slug+hash company part');
  const card = contactToVcard(contacts[0], { rev: '2026-07-09T00:00:00.000Z' });
  assert(/UID:studentcareer-jane-doe-[0-9a-f]{8}--acme-[0-9a-f]{8}\r\n/.test(card), 'UID = studentcareer-{uidPart(name)}--{uidPart(company)}');
...
function writeVcf(contacts, quality) {
  ...
  // repo. Anchored to the repo root (STUDENT_CAREER_AI), not process.cwd() — see the
  // generate-pdf.mjs precedent. Cheap lexical gate first…
  const relOut = relative(STUDENT_CAREER_AI, outPath);
  ...
  const repoReal = realpathSync(STUDENT_CAREER_AI);
