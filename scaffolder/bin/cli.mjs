#!/usr/bin/env node
// student-career-ai scaffolder — one-command install.
...
const REPO = "https://github.com/0xSamad/StudentCareerAI.git";
const LATEST_RELEASE = "https://api.github.com/repos/0xSamad/StudentCareerAI/releases/latest";
const NPM = process.platform === "win32" ? "npm.cmd" : "npm";

// student-career-ai is AI-agnostic: every one of these CLIs reads AGENTS.md and works
...
const USAGE = `student-career-ai — set up an AI job search workspace.

Usage:
  npx student-career-ai init [folder]    Create a new workspace (default: ./student-career-ai)

After setup, open your AI coding tool inside the folder and paste a job offer.
Docs: https://github.com/0xSamad/StudentCareerAI`;
...
async function latestTag() {
  try {
    const res = await fetch(LATEST_RELEASE, {
      headers: { "User-Agent": "student-career-ai-cli", Accept: "application/vnd.github+json" },
    });
...
async function main() {
  ...
  const target = dirArg || "student-career-ai";
  ...
  // Pretty path for messages: "./student-career-ai" for relative, as-is for absolute.
  ...
  console.log(`\n→ Cloning student-career-ai${tag ? ` @ ${tag}` : ""} into ${display} ...`);
  ...
  console.log(`\n✓ student-career-ai is ready in ${display}\n`);
  ...
  console.log("\nstudent-career-ai is AI-agnostic — Claude Code, Codex, Qwen, OpenCode, Copilot, Antigravity and Grok all work.");
