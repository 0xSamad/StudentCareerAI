#!/usr/bin/env node
/**
 * upskill.mjs — Aggregate skill-gap analyzer for student-career-ai (#1520, phase 1)
 *
...
const CV_FILE = join(STUDENT_CAREER_AI, 'cv.md');
const PROFILE_FILE = join(STUDENT_CAREER_AI, 'config/profile.yml');
...
function analyze(minReports) {
  ...
  for (const row of rows) {
    ...
    const candidates = [join(dirname(APPS_FILE), linkMatch[1]), join(STUDENT_CAREER_AI, linkMatch[1])];
...
if (urlTextIdx !== -1 || directUrl) {
  (async () => {
    ...
    if (!existsSync(activeCvFile)) {
      activeCvFile = join(STUDENT_CAREER_AI, 'cv-example.md');
    }
    if (existsSync(activeCvFile)) {
